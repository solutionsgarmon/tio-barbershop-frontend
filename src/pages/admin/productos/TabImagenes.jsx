...
import { updateBarbershop, updateProduct } from "../../../api/updates";
...
const TabImagenes = ({ productSelected }) => {
  ...
  const handleSubirImagen = async (file) => {
    ...
    try {
      ...
      await updateProduct(newValues, productSelected._id);
  ...
  const uploadImageFirestore = async (file) => {
    try {
      ...
      const pathImg = `products/${id}.${extension}`;
  ...
  return (
    <div>
      {productSelected !== null ? (
        ...
        <AlertWarning
          text={"Debe seleccionar un producto para modificar sus Imágenes"}
        />
      )}
    </div>
  );
