...
import { getBarbers, getBarbershops, getCitas, getCitasRegistro, getServices } from "../../../api/gets";
import { useAppContext } from "../../../context/AppProvider";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, PieChart, Pie, Cell, ResponsiveContainer } from "recharts";
import { Avatar, Stack, Box, Tooltip as MuiTooltip, Paper, FormControl, InputLabel, Select, MenuItem, Typography, Chip, IconButton } from "@mui/material";
...
const ReportesAdministrador = () => {
	...
	useEffect(() => {
		async function fetchData() {
			...
			const allCitasSinFiltrar = [...Citas, ...CitasRegistro];
			const allCitas = allCitasSinFiltrar.filter((cita) => cita.tipo_cita == "CITA");

			const uniqueDates = Array.from(new Set(allCitas.map((cita) => cita.fecha_asignada)));
	...
	const calcularCostoPorServicio = () => {
		const citasFiltradas = citas.filter(
			(cita) =>
				(selectedDateRange === "TODOS" || cita.fecha_asignada === selectedDateRange) &&
				(selectedStatus === "TODOS" || cita.estatus === selectedStatus) &&
				(selectedBarberoId === "TODOS" || cita.barbero_asignado === selectedBarberoId) // Filtrar por el _id del barbero
		);
		...
		citasFiltradas.forEach((cita) => {
			serviciosData[cita.nombre_servicio_asignado] += cita.costo || 0;
		});
	...
	const calcularCitasPorEstatus = () => {
		const completadas = citas.filter((cita) => cita.estatus === "COMPLETADA").length;
	...
	const customColors = ["#1f1f1f", "#82ca9d", "#ffc658", "#ff7f0e", "#ff0066", "#3399ff"];
	...
	const handleBarberoChange = (event) => {
		...
		const barberId = event.target.value === "TODOS" ? "TODOS" : barbers.find((barber) => barber.nombre === event.target.value)._id;
		...
		<Box>
			...
			>
				<Stack direction='row'>
					...
					<FormControl sx={{ mx: 0.5, minWidth: { xs: 150, sm: 250 } }}>
						...
						<Select value={selectedBarbero} onChange={handleBarberoChange} label='Barbero'>
					...
					<FormControl sx={{ mx: 0.5, minWidth: { xs: 130, sm: 150 } }}>
						...
						<Select value={selectedDateRange} onChange={handleDateRangeChange} label='Fecha'>
					...
					<FormControl sx={{ mx: 0.5, minWidth: { xs: 100, sm: 150 } }}>
						...
						<Select value={selectedStatus} onChange={handleStatusChange} label='Estatus'>
							<MenuItem value='TODOS'>Todos</MenuItem>
				...
				{selectedBarbero == "TODOS" && (
					<MuiTooltip title='Balance Total'>
						<Chip icon={<MonetizationOnIcon sx={{ width: 26, height: 26 }} />} label={totalGanancias} variant='outlined' sx={{ ml: 1 }} style={{ fontSize: "18px" }} />
					</MuiTooltip>
				)}
			</Paper>
			...
			<Paper sx={{ mb: 1, py: 2 }}>
				...
				>
					...
					{selectedDateRange == "TODOS" && selectedBarbero == "TODOS" && selectedStatus == "TODOS" && (
						<Box>
							<Box
								title={
									<Stack>
							...
							>
								...
								>
									...
									<PieChart width={230} height={230}>
										<Pie data={calcularCitasPorEstatus()} cx='50%' cy='50%' innerRadius={60} outerRadius={80} fill='#1f1f1f' dataKey='value' label>
											{calcularCitasPorEstatus().map((entry, index) => (
					...
					{selectedBarbero !== "TODOS" && (
						...
						>
							...
							/>
							<MuiTooltip title='Balance Total'>
								<Chip icon={<MonetizationOnIcon sx={{ width: 26, height: 26 }} />} label={totalGanancias} variant='outlined' sx={{ mt: 1, ml: 5 }} style={{ fontSize: "18px" }} />
							</MuiTooltip>
						</Box>
					)}
					{/* GRAFICA BARRAS*/}
					<ResponsiveContainer width='100%' height={400}>
						<BarChart data={[calcularCostoPorServicio()]} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
							...
							{services.map((service, index) => (
								<Bar key={service.nombre} dataKey={service.nombre} fill={customColors[index % customColors.length]} />
							))}
						</BarChart>
					</ResponsiveContainer>
				</Stack>
