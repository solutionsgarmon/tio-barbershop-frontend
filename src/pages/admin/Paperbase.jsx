...
import Descansos from "./descansos/Descansos";
import DescansosBarbero from "./descansos/Descansos";
	...
	},
	{
		...
		children: [
			...
			},
			{
				id: "Descansos",
				icon: <PunchClockIcon />,
				tabs: ["Descansos Pendientes", "Registro de Descansos"],
				component: <DescansosBarbero />,
			},
		],
	},
	// {
...
export default function Paperbase() {
	...
	const [firstCicle, setFirstCicle] = useState(true);

	const { windowWidth, indexTabSelected, setIndexTabSelected, sessionDataStorage } = useAppContext();
		...
		if (sessionDataStorage && firstCicle) {
			if (sessionDataStorage.rol == "ADMINISTRADOR") {
				...
				if (sessionDataStorage?.esAdmin === "NO") {
					...
					setDisplayComponent(<ReportesBarbero />);
				}
			}
			setFirstCicle(false);
		} else {
	...
	useEffect(() => {
		const categorySelected = categoriasMapear.find((cat) => cat.children.some((child) => child.id === idCategorieSelected));

		if (categorySelected) {
			const selectedChildren = categorySelected.children.find((child) => child.id === idCategorieSelected);
		...
		<ThemeProvider theme={theme}>
			...
			>
				...
				<Box sx={{ flex: 1, display: "flex", flexDirection: "column" }}>
					<Header onDrawerToggle={handleDrawerToggle} tabs={displayTabs} isSmUp={isSmUp} setIndexTabSelected={setIndexTabSelected} indexTabSelected={indexTabSelected} />
					{/* Ventana que cambia */}
					<Box
						component='main'
						sx={{
